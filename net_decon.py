...
class FractionallyStridedConvolutionNet(nn.Module):
    def __init__(self):
        super(FractionallyStridedConvolutionNet, self).__init__()

        # 4x4
        self.fsc1 = nn.Sequential(
            nn.ConvTranspose2d(in_channels=1, out_channels=128, kernel_size=4, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(128),
            nn.ReLU(True)
        )
        # 7x7
...
if __name__ == '__main__':
    ...
    logger = Logger('./logs/decon/v1/{}'.format(strtime))
    ...
    if args.with_reconstruction:
        reconstruction_model = FractionallyStridedConvolutionNet()
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        torch.save(model.state_dict(),
                   './models_decon/v1/{:03d}_model_dict_{}routing_reconstruction{}.pth'.format(epoch, args.routing_iterations,
