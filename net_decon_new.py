...
class ReconstructionNet(nn.Module):
    def __init__(self, n_dim=16, n_classes=10, n_channel_start=128):
        super(ReconstructionNet, self).__init__()
        self.n_dim = n_dim
        self.n_classes = n_classes
        self.n_channel_start = n_channel_start

        self.linear = nn.Linear(n_dim, n_channel_start * 4 * 4, bias=True)

        self.fsc1 = nn.Sequential(
            nn.ConvTranspose2d(in_channels=n_channel_start, out_channels=n_channel_start//2, kernel_size=4, stride=1, padding=0,
                               bias=False),
            nn.BatchNorm2d(n_channel_start//2),
            nn.ReLU(True)
        )
        self.fsc2 = nn.Sequential(
            nn.ConvTranspose2d(in_channels=n_channel_start//2, out_channels=n_channel_start//4, kernel_size=4, stride=2, padding=1,
                               bias=False),
            nn.BatchNorm2d(n_channel_start//4),
            nn.ReLU(True)
        )
        self.fsc3 = nn.Sequential(
            nn.ConvTranspose2d(in_channels=n_channel_start//4, out_channels=1, kernel_size=4, stride=2, padding=1, bias=False),
            nn.Tanh()
        )

    def forward(self, x, target):
        N, C, D = x.size()
        assert self.n_classes == C
        assert self.n_dim == D

        # Mask only the feature vector of correct label from the DigitCap
        ...
        # Reconstruct
        x = self.linear(x)
        x = x.view(N, self.n_channel_start, 4, 4) # 4x4
        x = self.fsc1(x) # 7x7
        x = self.fsc2(x) # 14x14
        x = self.fsc3(x) # 28x28
        return x
...
if __name__ == '__main__':
    ...
    logger = Logger('./logs/new_decon/v1/{}'.format(strtime))
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        torch.save(model.state_dict(),
                   './models_new_decon/v1/{:03d}_model_dict_{}routing_reconstruction{}.pth'.format(epoch, args.routing_iterations,
